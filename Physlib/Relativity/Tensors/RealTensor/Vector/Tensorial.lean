...
# Tensorial nature Lorentz Vectors

The instance of a tensor on Lorentz vectors, and
...
## Results related to the basis.

-/

open TensorSpecies Tensorial Tensor
...
lemma toTensor_basis_eq_tensor_basis {d : ℕ} (μ : Fin 1 ⊕ Fin d) :
    toTensor (Vector.basis μ) = Tensor.basis ![Color.up] (indexEquiv.symm μ) := by
  rw [← toTensor_symm_basis]
  simp

lemma basis_eq_map_tensor_basis {d} : Vector.basis =
    ((Tensor.basis
...
lemma tensor_basis_map_eq_basis_reindex {d} :
    ...
    Vector.basis.reindex indexEquiv.symm := by
...
lemma smul_eq_sum {d : ℕ} (i : Fin 1 ⊕ Fin d) (Λ : LorentzGroup d) (p : Vector d) :
  ...
  · intro r t h
    ...
    congr
    funext x
    simp only [Nat.succ_eq_add_one, Nat.reduceAdd, Vector.apply_smul]
    ring
  · intro t1 t2 h1 h2
